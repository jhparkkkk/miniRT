...
void	*mlx_int_parse_xpm(t_xvar *xvar,void *info,int info_size,char *(*f)())
{
		...
		if (!(line = f(info,&pos,info_size)) ||
						!(tab = mlx_int_str_to_wordtab(line)) || !(SIZEX = atoi(tab[0])) ||
						!(SIZEY = atoi(tab[1])) || !(nc = atoi(tab[2])) ||
						!(cpp = atoi(tab[3])) )
				RETURN;
		free(tab);
		tab = 0;
		...
		while (i--)
		{
				...
				{
						if (!(clip_data = malloc(4*SIZEX*SIZEY)) ||   ok, nice size ..
										!(clip_img = XCreateImage(xvar->display, xvar->visual,
														1, XYPixmap, 0, clip_data,
														SIZEX, SIZEY, 8, (SIZEX+7)/8)) )
								RETURN;
						memset(clip_data, 0xFF, 4*SIZEX*SIZEY);
				}
				*/
				if (method)
		...
		if (!(img = mlx_new_image(xvar,SIZEX,SIZEY)))
				RETURN;
		opp = img->bpp/8;
		...
		while (i--)
		{
				...
				while (x<SIZEX)
				{
						...
						/*
						if (col==-1)
								XPutPixel(clip_img, x, SIZEY-1-i, 0);
						else
...
void	*mlx_xpm_file_to_image(t_xvar *xvar,char *file,int *SIZEX,int *SIZEY)
{
		int	fd;
		int	size;
		char	*ptr;
...
void	*mlx_xpm_to_image(t_xvar *xvar,char **xpm_data,int *SIZEX,int *SIZEY)
{
		t_img	*img;
