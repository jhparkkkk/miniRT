...
/*   Updated: 2022/10/28 14:34:17 by cgosseli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	hit_cy(t_ray *ray, t_object *cy, double shadow, t_hit_point *hit)
{
	double		discr;
	double		tmp;
	double		hit_disk;
	...
	hit->a = hit->a - (tmp * tmp);
	hit->b = 2.0 * (vec_dot(ray->dir, hit->oc)
			- tmp * vec_dot(hit->oc, cy->dir));
