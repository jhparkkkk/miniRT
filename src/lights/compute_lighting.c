...
/*   Updated: 2022/09/27 16:38:20 by cgosseli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	compute_lighting(t_ray *ray, t_object *sp, t_world *world)
{
	...
	if (sp->specular_exponent != -1.0)
	{
		...
		if (r_dot_v > 0.0)
			// intensity += (world->light.intensity * (r_dot_v / (vec_len(reflect) * vec_len(view)))* sp->specular_exponent) ;
			intensity += world->light.intensity * pow(r_dot_v / (vec_len(reflect) * vec_len(view)), 1 / sp->specular_exponent);
	}
	return (intensity);
}