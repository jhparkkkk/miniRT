...
/*   Updated: 2022/10/28 17:29:12 by cgosseli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_light light)
{
		...
		&& shadow(hit.point, hit.vec_light, world, light))
	{
		...
		// {
			...
			{	
				// hit.rdotv = 0.0;
				hit.spec += (1.0 * pow(hit.rdotv / (vec_len(hit.reflect) * vec_len(hit.view)), 100.4));
				// hit.spec += light.intens *0.00001 * pow(hit.rdotv, 6.9);
